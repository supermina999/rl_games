...
class A2CAgent:
    def __init__(self, base_name, observation_space, action_space, config):
        ...
        self.old_mu_ph = tf.placeholder('float32', (None,) + action_space.shape, name = 'old_mu_ph')
        self.old_sigma_ph = tf.placeholder('float32', (None,) + action_space.shape, name = 'old_sigma_ph')
        self.old_neglogp_actions_ph = np.asarray([0]*self.num_actors, dtype=np.float32)
        self.rewards_ph = np.asarray([0]*self.num_actors, dtype=np.float32)
        self.old_values_ph = np.asarray([0]*self.num_actors, dtype=np.float32)
        self.advantages_ph = np.asarray([0]*self.num_actors, dtype=np.float32)
        self.learning_rate_ph = 1e-4
