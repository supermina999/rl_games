...
class SACAgent:
    ...
    def setup_qvalues(self, actions_num):
        ...
        self.actions, self.mean, self.log_prob, self.values = self.network(agent_critic_dict)
        self.actions, self.next_mean, self.next_log_prob, _ = self.network(agent_next_actions_dict, reuse=True)

        self.next_values = self.network(target_critic_dict)

        self.weights = tf.get_collection(tf.GraphKeys.TRAINABLE_VARIABLES, scope='agent/critic')
        self.target_weights = tf.get_collection(tf.GraphKeys.TRAINABLE_VARIABLES, scope='target/critic')
